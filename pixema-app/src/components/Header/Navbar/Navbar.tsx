...
const Navbar = ({ burger, open, closeBurger, openBurgerFunction }: INav) => {
    ...
    <>
      ...
      <div
        className={cn(
          "nav",
          burger && "burger-nav",
          open && "burger-nav__open"
        )}
      >
        {pathnames.map((path) => (
          <Link
            key={path.toLocaleLowerCase()}
            id={path.toLocaleLowerCase()}
            to={
              path.toLocaleLowerCase() === "home"
                ? "/"
                : path.toLocaleLowerCase()
            }
            onClick={() => setClick(!click)}
            className={cn(
              window.location.pathname === `/${path.toLocaleLowerCase()}` &&
                "nav__active",
              window.location.pathname === "/" && path === "Home"
                ? "nav__active"
                : undefined
            )}
            // тут надо посмотреть с перезагрузкой когда будет готов мэйн
          >
            <svg
              width="18"
              height="18"
